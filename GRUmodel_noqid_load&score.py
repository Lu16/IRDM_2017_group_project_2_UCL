...
with tf.device('/gpu:0'):
    with tf.Session() as sess:
        ...
        for m in range(1,5+1): # loop for training 5 folds
            # restore model
            print(m)
            saver.restore(sess, './GRU_model_noqid_' + str(m) + '/')
            ...
            for i in range(an):
                if i == an-1:
                    test_data = file_test[(i*batch_size):(i*batch_size+b), 2:]
                    ...
                    test_data = file_test[(i*batch_size):((i+1)*batch_size), 2:]
            ...
            test_score_file = open('GRU_noqid_score_' + str(m) + '.txt', "w", encoding="latin-1")
